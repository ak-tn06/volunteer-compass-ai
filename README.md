...
- **AI:** Claude (Sonnet), called through a local proxy server in live mode; realistic mock responses in demo mode (no API key required)
...
There are two ways to run this, and they're clearly separated in the code (see `server.js` comments and the "7A / 7B / 7C" sections in the HTML file's script).

**Demo mode (default, no setup required):** just open `Volunteer_Compass_AI_Prototype.html` directly in a browser — double-click it, no server or API key needed. All AI responses are realistic mock content built from small per-role and per-concern content tables, not live model output. A "Demo mode" indicator in the top navigation confirms this, and it's also stated on the welcome screen. This mode exists specifically so the product experience can be evaluated instantly, without any setup friction.

**Live mode (real Claude API):** requires the local proxy server, since calling `api.anthropic.com` directly from the browser isn't a safe pattern (see the security note below).

1. Make sure `server.js` and `Volunteer_Compass_AI_Prototype.html` are in the same folder.
2. Get an API key from [console.anthropic.com](https://console.anthropic.com).
3. In that folder, set the key as an environment variable:
   - Mac/Linux: `export ANTHROPIC_API_KEY=your-key-here`
   - Windows (cmd): `set ANTHROPIC_API_KEY=your-key-here`
4. Run `node server.js` (no npm install needed — it only uses Node's built-in modules).
5. Open `http://localhost:3000` in a browser.
6. Click the mode toggle in the top navigation to switch from "Demo mode" to "Live mode." Opening the file directly (`file://...`) and switching to live mode will not work — live mode needs the server running to reach Claude safely.

## Deployment Instructions (for a real pilot)

`server.js` already implements the backend-proxy pattern a real pilot would need — it just runs locally instead of on hosted infrastructure. To actually deploy this for a nonprofit to use:

1. Deploy `server.js` (or an equivalent small serverless function) to a host like Vercel, Netlify, or Render, with `ANTHROPIC_API_KEY` set as a server-side environment variable there instead of locally.
2. Serve `Volunteer_Compass_AI_Prototype.html` as the static frontend from the same platform.
3. Swap in the organization's real name and specific onboarding details in place of the demo placeholders.

No database, accounts, or additional infrastructure would be needed for the scope this tool currently covers — the local server and the production deployment use the same code path, just pointed at different hosting.
